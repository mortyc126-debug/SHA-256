/*
 * SP v3: Optimized Survey Propagation for large-scale random 3-SAT
 * =================================================================
 *
 * Core: 3-state cavity normalization (P_u + P_s + P_0, excludes P_c)
 * Speed: batch decimation (fraction per round), random edge update order
 * Scale: designed for n up to 20000
 *
 * Key parameters tuned:
 *   - Damping ρ=0.1 (light) with adaptive escalation
 *   - Decimation fraction: max(1, n_free * 0.005) per round
 *   - WalkSAT: SKC noise=57%, up to n*5000 flips
 *   - 3 full restarts
 *
 * Compile: gcc -O3 -march=native -o sp_v3 sp_v3.c -lm
 */

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
#include <time.h>

#define MAX_N       20000
#define MAX_CLAUSES 100000
#define MAX_K       3
#define MAX_DEGREE  300
#define SP_MAX_ITER 1000
#define SP_EPS      1e-4

static int n_vars, n_clauses;
static int cl_var[MAX_CLAUSES][MAX_K];
static int cl_sign[MAX_CLAUSES][MAX_K];
static int cl_active[MAX_CLAUSES];
static int var_fixed[MAX_N];

static int vlist[MAX_N][MAX_DEGREE];
static int vpos[MAX_N][MAX_DEGREE];
static int vdeg[MAX_N];

static double eta[MAX_CLAUSES][MAX_K];
static double W_plus[MAX_N], W_minus[MAX_N];

/* RNG - xoshiro256** */
static unsigned long long rng_s[4];
static inline unsigned long long rng_next(void) {
    unsigned long long s0=rng_s[0], s1=rng_s[1], s2=rng_s[2], s3=rng_s[3];
    unsigned long long r = ((s1*5)<<7|(s1*5)>>57)*9;
    unsigned long long t = s1<<17;
    s2^=s0; s3^=s1; s1^=s2; s0^=s3; s2^=t; s3=(s3<<45)|(s3>>19);
    rng_s[0]=s0; rng_s[1]=s1; rng_s[2]=s2; rng_s[3]=s3;
    return r;
}
static void rng_seed(unsigned long long s) {
    rng_s[0]=s; rng_s[1]=s*6364136223846793005ULL+1;
    rng_s[2]=s*1103515245ULL+12345; rng_s[3]=s^0xdeadbeefcafebabeULL;
    for(int i=0;i<20;i++) rng_next();
}
static double rng_double(void) {
    return (rng_next()>>11)*(1.0/9007199254740992.0);
}

static void generate(int n, double ratio, unsigned long long seed) {
    n_vars = n;
    n_clauses = (int)(ratio * n);
    if(n_clauses > MAX_CLAUSES) n_clauses = MAX_CLAUSES;
    rng_seed(seed);
    memset(vdeg, 0, sizeof(int)*n);
    for(int ci=0; ci<n_clauses; ci++) {
        int vs[3];
        vs[0] = rng_next() % n;
        do { vs[1] = rng_next() % n; } while(vs[1]==vs[0]);
        do { vs[2] = rng_next() % n; } while(vs[2]==vs[0] || vs[2]==vs[1]);
        for(int j=0; j<3; j++) {
            cl_var[ci][j] = vs[j];
            cl_sign[ci][j] = (rng_next()&1) ? 1 : -1;
            int v = vs[j];
            if(vdeg[v] < MAX_DEGREE) {
                vlist[v][vdeg[v]] = ci;
                vpos[v][vdeg[v]] = j;
                vdeg[v]++;
            }
        }
    }
    for(int ci=0; ci<n_clauses; ci++) cl_active[ci] = 1;
    memset(var_fixed, -1, sizeof(int)*n);
}

/* ============================================================
 * 3-STATE SP UPDATE (correct zero-temperature formulation)
 *
 * η_{a→i} = ∏_{j∈a\i} [ P_u(j,a) / (P_u + P_s + P_0)(j,a) ]
 *
 * P_u = (1-∏_opp)(∏_same)   = pressure to unsatisfy clause a
 * P_s = (1-∏_same)(∏_opp)   = pressure to satisfy clause a
 * P_0 = (∏_same)(∏_opp)     = no pressure (free)
 * ============================================================ */

/* Edge list for random update order */
static int edge_ci[MAX_CLAUSES * MAX_K];
static int edge_pos[MAX_CLAUSES * MAX_K];
static int n_edges;

static void build_edge_list(void) {
    n_edges = 0;
    for(int ci=0; ci<n_clauses; ci++) {
        if(!cl_active[ci]) continue;
        for(int p=0; p<MAX_K; p++) {
            if(var_fixed[cl_var[ci][p]] >= 0) continue;
            edge_ci[n_edges] = ci;
            edge_pos[n_edges] = p;
            n_edges++;
        }
    }
}

static void shuffle_edges(void) {
    for(int i=n_edges-1; i>0; i--) {
        int j = rng_next() % (i+1);
        int tc = edge_ci[i]; edge_ci[i] = edge_ci[j]; edge_ci[j] = tc;
        int tp = edge_pos[i]; edge_pos[i] = edge_pos[j]; edge_pos[j] = tp;
    }
}

static inline double sp_compute_edge(int ci, int pos_i) {
    double product = 1.0;

    for(int pj = 0; pj < MAX_K; pj++) {
        if(pj == pos_i) continue;
        int j = cl_var[ci][pj];

        if(var_fixed[j] >= 0) {
            int sj = cl_sign[ci][pj];
            if((sj==1 && var_fixed[j]==1) || (sj==-1 && var_fixed[j]==0))
                return 0.0;
            continue;
        }

        int sign_j_in_a = cl_sign[ci][pj];
        double prod_s = 1.0, prod_u = 1.0;

        for(int d = 0; d < vdeg[j]; d++) {
            int bi = vlist[j][d], bp = vpos[j][d];
            if(bi == ci || !cl_active[bi]) continue;

            /* Check if clause b is satisfied by a fixed var */
            int b_sat = 0;
            for(int k=0; k<MAX_K; k++) {
                int vk = cl_var[bi][k];
                if(var_fixed[vk] >= 0) {
                    int sk = cl_sign[bi][k];
                    if((sk==1 && var_fixed[vk]==1) || (sk==-1 && var_fixed[vk]==0))
                        { b_sat=1; break; }
                }
            }
            if(b_sat) continue;

            double eta_b = eta[bi][bp];
            if(cl_sign[bi][bp] == sign_j_in_a)
                prod_s *= (1.0 - eta_b);
            else
                prod_u *= (1.0 - eta_b);
        }

        double P_u = (1.0 - prod_u) * prod_s;
        double P_s = (1.0 - prod_s) * prod_u;
        double P_0 = prod_u * prod_s;
        double denom = P_u + P_s + P_0;

        product *= (denom > 1e-15) ? (P_u / denom) : 0.0;
    }

    return product;
}

/* One SP iteration with random edge order and damping */
static double sp_iterate(double rho) {
    shuffle_edges();
    double max_change = 0;

    for(int e = 0; e < n_edges; e++) {
        int ci = edge_ci[e], p = edge_pos[e];
        if(!cl_active[ci] || var_fixed[cl_var[ci][p]] >= 0) continue;

        double nv = sp_compute_edge(ci, p);
        double ov = eta[ci][p];
        double up = rho * ov + (1.0 - rho) * nv;
        double ch = fabs(up - ov);
        if(ch > max_change) max_change = ch;
        eta[ci][p] = up;
    }
    return max_change;
}

/* Converge SP with adaptive damping */
static int sp_converge(int seed_hint) {
    build_edge_list();

    /* Try with increasing damping */
    double rhos[] = {0.1, 0.2, 0.4, 0.6, 0.8};
    int attempts[] = {SP_MAX_ITER, 300, 300, 200, 200};

    for(int s = 0; s < 5; s++) {
        for(int iter = 0; iter < attempts[s]; iter++) {
            double ch = sp_iterate(rhos[s]);
            if(ch < SP_EPS) return 1;
        }
    }

    /* Reinitialize and retry once */
    rng_seed(seed_hint * 10007ULL + 31337ULL);
    for(int ci=0; ci<n_clauses; ci++) {
        if(!cl_active[ci]) continue;
        for(int j=0; j<MAX_K; j++) {
            if(var_fixed[cl_var[ci][j]] >= 0) continue;
            eta[ci][j] = rng_double();
        }
    }
    build_edge_list();
    for(int s = 0; s < 3; s++) {
        for(int iter = 0; iter < 500; iter++) {
            double ch = sp_iterate(rhos[s]);
            if(ch < SP_EPS) return 1;
        }
    }
    return 0;
}

/* Bias computation (3-state) */
static void sp_compute_bias(void) {
    for(int i = 0; i < n_vars; i++) {
        W_plus[i] = W_minus[i] = 0;
        if(var_fixed[i] >= 0) continue;

        double prod_plus = 1.0, prod_minus = 1.0;
        for(int d = 0; d < vdeg[i]; d++) {
            int ci = vlist[i][d], p = vpos[i][d];
            if(!cl_active[ci]) continue;
            int sat = 0;
            for(int k=0; k<MAX_K; k++) {
                int vk = cl_var[ci][k];
                if(vk != i && var_fixed[vk] >= 0) {
                    int sk = cl_sign[ci][k];
                    if((sk==1 && var_fixed[vk]==1) || (sk==-1 && var_fixed[vk]==0))
                        { sat=1; break; }
                }
            }
            if(sat) continue;
            double e = eta[ci][p];
            if(cl_sign[ci][p] == 1) prod_plus *= (1.0 - e);
            else prod_minus *= (1.0 - e);
        }

        double pi_plus  = (1.0 - prod_plus) * prod_minus;
        double pi_minus = (1.0 - prod_minus) * prod_plus;
        double pi_zero  = prod_plus * prod_minus;
        double total = pi_plus + pi_minus + pi_zero;

        if(total > 1e-15) {
            W_plus[i]  = pi_plus / total;
            W_minus[i] = pi_minus / total;
        }
    }
}

/* Unit propagation */
static int unit_propagate(void) {
    int changed = 1;
    while(changed) {
        changed = 0;
        for(int ci=0; ci<n_clauses; ci++) {
            if(!cl_active[ci]) continue;
            int sat=0, fc=0, fv=-1, fs=0;
            for(int j=0; j<3; j++) {
                int v = cl_var[ci][j], s = cl_sign[ci][j];
                if(var_fixed[v] >= 0) {
                    if((s==1 && var_fixed[v]==1) || (s==-1 && var_fixed[v]==0))
                        sat = 1;
                } else {
                    fc++; fv=v; fs=s;
                }
            }
            if(sat) { cl_active[ci]=0; continue; }
            if(fc==0) return 1; /* contradiction */
            if(fc==1) { var_fixed[fv] = (fs==1) ? 1 : 0; changed=1; }
        }
    }
    return 0;
}

/* WalkSAT with SKC heuristic */
static int walksat(int max_flips) {
    int a[MAX_N], n = n_vars, m = n_clauses;
    for(int v=0; v<n; v++)
        a[v] = (var_fixed[v]>=0) ? var_fixed[v] : ((rng_next()&1) ? 1 : 0);

    int *sc = (int*)calloc(m, sizeof(int));
    for(int ci=0; ci<m; ci++)
        for(int j=0; j<3; j++) {
            int v = cl_var[ci][j], s = cl_sign[ci][j];
            if((s==1 && a[v]==1) || (s==-1 && a[v]==0)) sc[ci]++;
        }

    int *ulist = (int*)malloc(m * sizeof(int));
    int *upos = (int*)malloc(m * sizeof(int));
    int nu = 0;
    memset(upos, -1, sizeof(int)*m);
    for(int ci=0; ci<m; ci++)
        if(sc[ci]==0) { upos[ci]=nu; ulist[nu++]=ci; }

    for(int f=0; f<max_flips && nu>0; f++) {
        int ci = ulist[rng_next() % nu];
        int bv = cl_var[ci][0], bb = m+1, zb = -1;
        for(int j=0; j<3; j++) {
            int v = cl_var[ci][j], br = 0;
            for(int d=0; d<vdeg[v]; d++) {
                int oci = vlist[v][d], opos = vpos[v][d];
                int os = cl_sign[oci][opos];
                if(((os==1 && a[v]==1) || (os==-1 && a[v]==0)) && sc[oci]==1)
                    br++;
            }
            if(br == 0) { zb = v; break; }
            if(br < bb) { bb = br; bv = v; }
        }

        int fv;
        if(zb >= 0) fv = zb;
        else if(rng_next() % 100 < 57) fv = cl_var[ci][rng_next() % 3];
        else fv = bv;

        int old = a[fv], nw = 1 - old;
        a[fv] = nw;

        for(int d=0; d<vdeg[fv]; d++) {
            int oci = vlist[fv][d], opos = vpos[fv][d];
            int os = cl_sign[oci][opos];
            int was = ((os==1 && old==1) || (os==-1 && old==0));
            int now = ((os==1 && nw==1) || (os==-1 && nw==0));
            if(was && !now) {
                sc[oci]--;
                if(sc[oci]==0) { upos[oci]=nu; ulist[nu++]=oci; }
            } else if(!was && now) {
                sc[oci]++;
                if(sc[oci]==1) {
                    int p = upos[oci];
                    if(p>=0 && p<nu) {
                        int l = ulist[nu-1];
                        ulist[p] = l; upos[l] = p; upos[oci] = -1; nu--;
                    }
                }
            }
        }
    }

    for(int v=0; v<n; v++) var_fixed[v] = a[v];
    int result = m - nu;
    free(sc); free(ulist); free(upos);
    return result;
}

/* ============================================================
 * SP-GUIDED DECIMATION
 * ============================================================ */

static int saved_active[MAX_CLAUSES];
static int saved_fixed[MAX_N];

static int sp_solve(int nn) {
    int n = nn, m = n_clauses;

    if(unit_propagate()) return 0;

    /* Save state for restarts */
    memcpy(saved_active, cl_active, sizeof(int)*m);
    memcpy(saved_fixed, var_fixed, sizeof(int)*n);

    for(int restart = 0; restart < 3; restart++) {
        if(restart > 0) {
            memcpy(cl_active, saved_active, sizeof(int)*m);
            memcpy(var_fixed, saved_fixed, sizeof(int)*n);
        }

        int n_fixed = 0;
        for(int v=0; v<n; v++) if(var_fixed[v]>=0) n_fixed++;

        /* Initialize surveys randomly */
        rng_seed(42ULL + restart * 9973ULL + (unsigned long long)n * 13);
        for(int ci=0; ci<m; ci++)
            for(int j=0; j<MAX_K; j++)
                eta[ci][j] = rng_double();

        int sp_rounds = 0;
        while(n_fixed < n) {
            int conv = sp_converge(n_fixed * 31 + restart * 7);
            if(!conv) break;

            /* Check trivialization */
            double max_eta = 0, sum_eta = 0; int cnt = 0;
            for(int ci=0; ci<m; ci++) {
                if(!cl_active[ci]) continue;
                for(int j=0; j<MAX_K; j++) {
                    if(var_fixed[cl_var[ci][j]] >= 0) continue;
                    if(eta[ci][j] > max_eta) max_eta = eta[ci][j];
                    sum_eta += eta[ci][j]; cnt++;
                }
            }

            if(max_eta < 0.01 || cnt == 0) break; /* trivialized → WalkSAT */

            /* Compute bias */
            sp_compute_bias();

            /* Find n_fix most biased free variables */
            int n_free = n - n_fixed;
            int n_fix = n_free / 200;  /* 0.5% per round */
            if(n_fix < 1) n_fix = 1;
            if(n_fix > 50) n_fix = 50;

            for(int f = 0; f < n_fix; f++) {
                double best_bias = -1;
                int best_var = -1, best_val = 0;
                for(int v=0; v<n; v++) {
                    if(var_fixed[v] >= 0) continue;
                    double b = fabs(W_plus[v] - W_minus[v]);
                    if(b > best_bias) {
                        best_bias = b; best_var = v;
                        best_val = (W_plus[v] > W_minus[v]) ? 1 : 0;
                    }
                }
                if(best_var < 0 || best_bias < 0.001) break;

                var_fixed[best_var] = best_val;
                n_fixed++;

                /* Deactivate satisfied clauses */
                for(int d=0; d<vdeg[best_var]; d++) {
                    int ci = vlist[best_var][d], p = vpos[best_var][d];
                    if(!cl_active[ci]) continue;
                    int s = cl_sign[ci][p];
                    if((s==1 && best_val==1) || (s==-1 && best_val==0))
                        cl_active[ci] = 0;
                }
            }

            if(unit_propagate()) break;
            n_fixed = 0;
            for(int v=0; v<n; v++) if(var_fixed[v]>=0) n_fixed++;
            sp_rounds++;
        }

        /* WalkSAT finish */
        int flips = n * 5000;
        if(flips > 20000000) flips = 20000000;

        for(int ws = 0; ws < 3; ws++) {
            rng_seed(restart * 77777ULL + ws * 12345ULL + n_fixed);
            int sat = walksat(flips);
            if(sat == m) return sat;
        }
    }

    /* Final hail-mary */
    rng_seed(999999ULL);
    return walksat(n_vars * 10000);
}

static int evaluate_all(void) {
    int sat = 0;
    for(int ci=0; ci<n_clauses; ci++) {
        for(int j=0; j<3; j++) {
            int v = cl_var[ci][j], s = cl_sign[ci][j];
            int val = (var_fixed[v]>=0) ? var_fixed[v] : 0;
            if((s==1 && val==1) || (s==-1 && val==0)) { sat++; break; }
        }
    }
    return sat;
}

/* ============================================================
 * BENCHMARK
 * ============================================================ */
int main(void) {
    printf("====================================================\n");
    printf("SP v3: Optimized 3-state SP + batch decimation\n");
    printf("====================================================\n");
    printf("3-state normalization, random edge order, adaptive damping\n");
    printf("Batch decimation (0.5%%), 3 restarts, WalkSAT finish\n\n");

    int test_n[] = {100, 200, 300, 500, 750, 1000, 2000, 3000, 5000, 10000};
    int sizes = 10;

    printf("%6s | %5s | %6s | %8s | %s\n",
           "n", "total", "solved", "time_ms", "rate");
    printf("-------+-------+--------+----------+------\n");

    for(int ti=0; ti<sizes; ti++) {
        int nn = test_n[ti];
        int n_inst;
        if(nn <= 200) n_inst = 20;
        else if(nn <= 500) n_inst = 10;
        else if(nn <= 2000) n_inst = 5;
        else n_inst = 3;

        int solved = 0, total = 0;
        double total_ms = 0;

        for(int seed=0; seed < n_inst * 3 && total < n_inst; seed++) {
            generate(nn, 4.267, 11000000ULL + seed);

            clock_t t0 = clock();
            rng_seed(seed * 31337ULL);
            int sat = sp_solve(nn);
            clock_t t1 = clock();
            double ms = (double)(t1-t0)*1000.0/CLOCKS_PER_SEC;
            total_ms += ms;
            total++;
            if(sat == n_clauses) solved++;
        }

        printf("%6d | %2d/%2d | %4d   | %7.0fms | %3.0f%%\n",
               nn, solved, total, solved, total_ms/total,
               100.0*solved/total);
        fflush(stdout);

        /* Stop if taking too long and zero solve rate */
        if(total_ms/total > 60000 && solved == 0) {
            printf("  (stopping: too slow with 0%% solve rate)\n");
            break;
        }
    }

    printf("\n=== COMPARISON ===\n");
    printf("sp_correct (4-state):  n=750 40%%, n=1000 0%%\n");
    printf("sp_v2 (3-state old):   n=500 40%%, n=750 40%%, n=1000 20%%, n=2000 0%%\n");
    printf("sp_v3 (this):          (results above)\n");
    printf("Literature:            n=100000+ at threshold\n");

    return 0;
}

...
| **blake2b** | -0.10 +0.37 +1.00 -0.72 -1.66 -0.68 -0.36 -0.51 +0.14 | **6/9** | **5/9** | **≈DRIFT** |
| **blake2s** | +0.36 +0.60 -0.22 -0.60 -0.29 -1.39 -0.69 -1.09 -0.16 | **7/9** | **5/9** | **≈DRIFT** |

**Вердикт**: все individual z-значения совпадают; SHA-2 family z ≈ −2.5 и
sign-test p подтверждены. **Два расхождения** в счёте neg/9 для
BLAKE2b/BLAKE2s (6 и 7 вместо 5). Интерпретация «no signal» (p ≫ 0.05)
сохраняется: binomial для 6/9 → p≈0.25; для 7/9 → p≈0.18 — обе >> 0.05.

**Рекомендация**: исправить в
`unified/03_vol_III/02_min_entropy_chi2.md` (таблица §III.2.2, SHARP §4)
и в `research/min_entropy/SHARP_FINDINGS.md` строки blake2b/blake2s:
`5/9 → 6/9` и `5/9 → 7/9` соответственно. Содержательные выводы остаются.
...
| IT-4.Q7D-R500 | 8 | 0 | 0 |
| IT-1.3 cross-hash | 5 хэшей | 2 (blake2b/s neg counts) | 0 |
| IT-2 chimera | 7 | 0 | 0 |

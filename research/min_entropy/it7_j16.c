/* Dedicated j=16 test: FIXED flip bit, N=25M train + 25M test */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <string.h>
...
static uint64_t xs[2]={0xDEADCAFE12345678,0xABCD9876FEDCBA01};
...
int main(void){
    ...
    fprintf(stderr,"Test %dM (j=%d)...\n",N_HALF/1000000,FIXED_J);
    double ss=0,sh=0,ssh=0,ss2=0,sh2=0; long ct=0;
    int bsign[50]; int nb=0; double bs=0,bh=0,bsh2=0,bss2=0,bhh2=0; long bc=0;
    for(long i=0;i<N_HALF;i++){
        fr(msg,64);
        for(int w=0;w<16;w++)b1[w]=((uint32_t)msg[w*4]<<24)|((uint32_t)msg[w*4+1]<<16)|((uint32_t)msg[w*4+2]<<8)|msg[w*4+3];
        memcpy(b2,b1,64); b2[0]^=(1u<<(31-FIXED_J));
        ...
        bs+=score;bh+=hw;bsh2+=score*hw;bss2+=score*score;bhh2+=(double)hw*hw;bc++;
        if(bc>=500000){double ms=bs/bc,mh=bh/bc,cv=bsh2/bc-ms*mh;
            double vs=bss2/bc-ms*ms,vh=bhh2/bc-mh*mh;
            double corr=(vs>0&&vh>0)?cv/sqrt(vs*vh):0;
            bsign[nb++]=(corr>0)?1:0;bs=bh=bsh2=bss2=bhh2=0;bc=0;}
        if((i+1)%5000000==0)fprintf(stderr,"  %ldM\n",(i+1)/1000000);
    }
    free(M);
    double ms=ss/ct,mh=sh/ct,cv=ssh/ct-ms*mh,vs=ss2/ct-ms*ms,vh=sh2/ct-mh*mh;
    double corr=(vs>0&&vh>0)?cv/sqrt(vs*vh):0;
    double z=corr*sqrt((double)ct-2);
    int np=0;for(int i=0;i<nb;i++)np+=bsign[i];

    printf("j=%d  N_test=%ld  corr=%.10f  z=%.4f  blocks_pos=%d/%d  mean_hw=%.6f\n",
           FIXED_J,ct,corr,z,np,nb,mh);
    return 0;
}

...
def hash_to_bits(inputs, hash_fn, digest_size=32):
    """Apply hash to each input, extract first 256 bits as (N, 256) uint8 array."""
    N = len(inputs)
    bits = np.zeros((N, 256), dtype=np.uint8)
    for i, m in enumerate(inputs):
        d = hash_fn(m)
        if len(d) > 32: d = d[:32]
        elif len(d) < 32: d = d + bytes(32 - len(d))
...
def omega_k_fast(chi_arr, target_bits, f_arr):
    ...
    N = target_bits.shape[0]
    sN = math.sqrt(N)
    f_pm = f_arr.astype(np.int8) * 2 - 1
    target_pm = target_bits.astype(np.int8) * 2 - 1  # (N, 256)

    # All z_out at once: chi_arr (n_tup × N) @ target_pm (N × 256) → (n_tup × 256)
    # int8 matmul → use float32 for BLAS
    z_in = (chi_arr.astype(np.float32) @ f_pm.astype(np.float32)) / sN  # (n_tup,)
    z_out_matrix = (chi_arr.astype(np.float32) @ target_pm.astype(np.float32)) / sN  # (n_tup, 256)

    # chain_z(b) = sum over S of z_in(S) × z_out(S, b) / sqrt(N)
    chain_z = (z_in[:, None] * z_out_matrix).sum(axis=0) / sN  # (256,)

    # direct_z(b) = f · target_b / sqrt(N)
...
def ro_null_band(inputs, state1_bits, f_arr, chi_arr, n_realizations=10):
    ...
    for r in range(n_realizations):
        ...
        omega, _, _, _ = omega_k_fast(chi_arr, bits, f_arr)
        omegas.append(omega)
...
def run(hash_name, N_samples=None, feature_name='bit5_max', verbose=True):
    """Main entry point."""
    # Get hash function
    HASH_MAP = {
        'md5':      (hashlib.md5, 16),
    ...
    if verbose: print(f"\n# Phase 1: hash inputs...")
    target_bits = hash_to_bits(inputs, hash_fn)
    if verbose: print(f"  time: {time.time()-t0:.0f}s")
    ...
    for k in [1, 2, 3]:
        ...
        omega, ss, dz, cz = omega_k_fast(chi_arr, target_bits, fa)

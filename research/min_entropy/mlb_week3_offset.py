...
OUT = os.path.join(HERE, 'mlb_week3_offset.json')

K = 20_000_000
THRESHOLD = 2_000_000
N_UNIFORM_PAIRS = 30_000


def compute_keys_only(W0_start, W0_end):
    N = W0_end - W0_start
    block1 = np.zeros((N, 16), dtype=ch.U32)
    block1[:, 0] = np.arange(W0_start + 50_000_000, W0_end + 50_000_000, dtype=ch.U32)
...
def compute_state1_for_indices(W0_indices):
    N = len(W0_indices)
    block1 = np.zeros((N, 16), dtype=ch.U32)
    block1[:, 0] = np.array(W0_indices, dtype=ch.U32) + np.uint32(50_000_000)
    U32, MASK = ch.U32, ch.MASK
    W = np.empty((N, 64), dtype=U32); W[:, :16] = block1
    for t in range(16, 64):
        W[:, t] = (ch.sigma1(W[:, t-2]) + W[:, t-7]

/*
 * IT-9D: End-to-end training — score from block-1 features,
 * target = HASH diff (after BOTH blocks).
 *
 * IT-9/9C trained score to predict block-1 diff. Wrong target.
 * Now: train M_in on block-1 state_diff features, but target =
 * HW(hash_diff) = what we actually need for collision.
 *
 * If corr > 0 → some block-1 features predict hash_diff through block 2.
 * The score learns WHICH features survive the second mixing.
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
static void sha256_2block(const uint32_t msg[16],uint32_t hash[8]){
    uint32_t st[8];compress_block(msg,IV,st);
    uint32_t pad[16]={0};pad[0]=0x80000000;pad[15]=512;
    compress_block(pad,st,hash);}
static uint64_t xs[2]={0xE2ECAFE00010002ULL,0xDEADE2EBEEF1234ULL};
...
int main(void){
    ...
    uint8_t msg[64];uint32_t b1[16],b2[16],s1[8],s2[8],h1[8],h2[8];
    int8_t Y[NBITS];

    /* TRAIN: features = block-1 state_diff, target = HASH diff HW */
    fprintf(stderr,"E2E Training: block1_diff features → hash_diff target...\n");
    long n_cw=0;
    ...
    fprintf(stderr,"Trained E2E. n_cw=%ld\n",n_cw);
    ...
    printf("=== IT-9D: End-to-end (block1 features → hash target) ===\n");
    printf("c-world test: %ld\n\n",a_hash.n);
    double ch=ac(&a_hash),zh=ch*sqrt((double)a_hash.n-2);
    double cb=ac(&a_b1),zb=cb*sqrt((double)a_b1.n-2);
    printf("Score vs HASH diff HW:    corr=%+.8f z=%+.2f mean=%.2f\n",ch,zh,a_hash.sh/a_hash.n);
    printf("Score vs block-1 diff HW: corr=%+.8f z=%+.2f mean=%.2f\n",cb,zb,a_b1.sh/a_b1.n);
    printf("\nIT-9C (wrong target): hash corr = +0.000164 z=+0.44\n");
    printf("IT-9D (right target): hash corr = %+.6f z=%+.2f\n",ch,zh);

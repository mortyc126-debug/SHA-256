/* Quick test: c-world Walsh-2 at r=64 (full block 1) */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <string.h>
...
static uint64_t xs[2]={0x64640064CAFE0001ULL,0xDEAD6464BEEF1234ULL};
...
int main(void){
    ...
    int rlist[]={16, 32, 48, 64};
    for(int ri=0;ri<4;ri++){
        int R=rlist[ri];
        /* Train on c-world at this round */
        double M[NBITS*NBITS]; memset(M,0,sizeof(M)); long ntr=0;
        for(long i=0;i<N_HALF;i++){
            ...
            for(int a=0;a<NBITS;a++)for(int bb=0;bb<NBITS;bb++)M[a*NBITS+bb]+=(double)Y[a]*Y[bb]*tgt;
            ntr++;
        }
        double sq=sqrt((double)ntr),tr=0;
        for(int i=0;i<NBITS*NBITS;i++)M[i]/=sq;
        for(int i=0;i<NBITS;i++)tr+=M[i*NBITS+i];
        /* Test */
        Acc ae={};
        /* Also test prediction on ALL 8 registers simultaneously */
        Acc a_full={}; /* score vs HW of FULL 256-bit state_diff */
        for(long i=0;i<N_HALF;i++){
            fr(msg,64);
            for(int w=0;w<16;w++)b1[w]=((uint32_t)msg[w*4]<<24)|((uint32_t)msg[w*4+1]<<16)|((uint32_t)msg[w*4+2]<<8)|msg[w*4+3];
            int j=xr()&31;memcpy(b2,b1,64);b2[0]^=(1u<<(31-j));
            ...
            double sc=(Q-tr)/2.0;
            aa(&ae,sc,(double)hw_e);
            aa(&a_full,sc,(double)hw_all);
        }
        printf("r=%d: train_cw=%ld test_cw=%ld\n",R,ntr,ae.n);
        printf("  e-register:  corr=%+.6f z=%+.2f mean_hw=%.2f\n",ac(&ae),ac(&ae)*sqrt((double)ae.n-2),ae.sh/ae.n);
        printf("  full 256-bit: corr=%+.6f z=%+.2f mean_hw=%.2f\n",ac(&a_full),ac(&a_full)*sqrt((double)a_full.n-2),a_full.sh/a_full.n);
    }
    return 0;
}

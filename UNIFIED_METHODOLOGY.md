...
### ⚠️ КРИТИЧЕСКАЯ ПРЕАМБУЛА (читать ПЕРВОЙ!)

**Программа Prismatic выполнена и завершена** (Sessions 1-69, 2026-04). Главный вывод:

> SHA-256 неуязвима для всех известных классических методов **не из-за нехватки попыток**, а из-за **математически точной калибровки** к 2^128 brute-force барьеру. 9 ортогональных attack frameworks конвергируют на один и тот же wall ~46 раундов; SHA имеет 64 раунда (margin 28%).

**Не повторять**: всё в разделе "PRISMATIC PROGRAM — ИТОГ 69 СЕССИЙ" (поиск по строке). Конкретно:
- z3 SAT collision (TIMEOUT T ≥ 5-8)
- Boomerang/Rebound (cap 46r in literature)
- LLL/XL/Gröbner (10^25 monomials at T=4)
- Multi-block random (birthday wall)
- Bit-permutation symmetries (NONE)
- Mod-q invariants (artefact only)
- Weak states (CV=0.06, no exploit)
- TDA / persistent homology (trivial)
- Joint compression (independent)
- 2-adic / prismatic lifting (ROTR не truncation-compatible)

**Что РАБОТАЕТ как infrastructure**: 62 теоремы в `research/prismatic/` — Lucas-XOR nilpotency (24.1), carry degree law (33.1), deterministic MSB transitions (67.1, единственная доказательная differential теорема), weak channel asymmetry (66.1: c/d/h slow vs e/a fast).

**Composition Lemma**: Per-round advantage линейна → AttackCost ≥ 2^(T·log₂(1/p)). При p=1/4, T=64: 2^128. Это и есть math reason "почему SHA не ломается".
...
5. **Раздел "PRISMATIC PROGRAM — ИТОГ 69 СЕССИЙ"** — итог 69-сессионной программы анализа.
...
**Для продолжения программы** (после scoping-сессии 2026-04 осталось 2 направления):
1. Wang extension за r=17 (T_BARRIER_EQUALS_SCHEDULE — реальная стена, методичка 1300+ экспериментов не пробила).
2. Signal amplification у block-2 [IT-4.S4].

**Закрыто 2026-04 сессией scoping** (см. §II.8.3 и `research/qt_minimal/`):
- ⊘SCOPED: MITM O(2⁸⁰) — "хуже Wang chain" по самой методичке, даёт state[17] near-collision, не full SHA-256 collision.
- ⊘SCOPED: Q∩T framework как отдельная алгебра — z3 BitVec SMT уже реализует это нативно через bit-vector theory.
- ⊘SCOPED: Cohomology probe (carry Jacobian deficit) — подтверждает §II.7.7 (589/592), но ≈1 бит всегда тривиален (MSB mod 2^n), остальное state-dependent = Wang territory.
- ⊘SCOPED: ANF early-verify beyond 7.6× (§132 extension) — fundamental ceiling `V/L` от sequential carry, triple-stacking ortho-check работает но дорог. Методичка §133 сама зафиксировала потолок.
- ⊘ROLL: Ω_k для SHA-3/BLAKE — chi_arr artifact (§III.7), направление dead-end.
...
| **⊘SCOPED** | Направление эмпирически закрыто через scoping-тесты (не повторять) | Q_QT_SOLVER, Q_MITM_STATE16 (§II.8.3) |
...
| **Jacobian carry deficit (R-invariant)** | n=32: 4.84 (≈3.8 без MSB); conserved через R | ⚡VER | §II.8.3, 2026-04 |
| **z3 BitVec phase transition на mini-SHA** | R=6 полный хеш = 30мс; R=8 full = TIMEOUT 120с | ⚡VER | §II.8.3, 2026-04 |
...
### Prismatic Program (Sessions 1-69, 2026-04, full closure)
- **Direct attacks**: z3 (T ≤ 5-8 timeout), Boomerang (cap 46r), Rebound (cost 2^120), LLL/XL (10^25 monomials at T=4), multi-block random (birthday wall) — все 5 frameworks tested ✗NEG.
- **Structural attacks**: bit-permutation symmetries ✗, mod-q invariants ✗, weak states (CV=0.06) ✗, TDA ✗, joint compression ✗, cycle structure ✗.
- **Indirect angles**: subspace asymmetry → структурный bound, conservation laws → 3.5σ артефакт mod 257, free-start → trivially solvable T ≤ 4, 2-adic lifting ⊘ ROTR не truncation-compatible (Theorem 40.1).
- **Composition Lemma** (мета): per-round advantage линейна, per-round разрушение экспоненциально → AttackCost(64) = 2^128 = brute force.
- **Calibration Principle**: 64 раунда = exact calibration к 2^128.
- **Не повторять**: см. подробный список в секции "PRISMATIC PROGRAM — ИТОГ 69 СЕССИЙ" (ниже).
...
- **Extension Wang-chain за r=17** ?OPEN — T_BARRIER_EQUALS_SCHEDULE, реальная стена, **единственный остающийся приоритет 1**
...
### Scoping-закрытые 2026-04 (не повторять, см. §II.8.3)
- ⊘SCOPED MITM O(2⁸⁰) — "хуже Wang chain" по самой v20 (Раздел 88)
- ⊘SCOPED Q∩T framework — z3 BitVec уже реализует через bit-vector theory
- ⊘SCOPED Cohomology probe — R-invariant но тривиальный (MSB) / state-dependent (Wang)
- ⊘SCOPED ANF early-verify beyond §132 — fundamental ceiling V/L, методичка §133 сама зафиксировала 9× потолок
- ⊘SCOPED Persistent kernel / Carry-linear-invariant — true exploration, формализовали уже известную Wang-chain структуру на языке линейной алгебры, нового attack не даёт
...
- ⊘ROLL Ω_k для SHA-3/BLAKE (chi_arr artifact, §III.7)

## ТЕКУЩАЯ ФРОНТЬЕРНАЯ ЗАДАЧА

**Главный фокус** (по состоянию на апрель 2026, после 4-way scoping-сессии):
1. Wang extension за r=17 (если вообще есть идея — T_BARRIER_EQUALS_SCHEDULE подтверждён 9 независимыми методами, §II.9.5)
2. Signal amplification у block-2 [IT-4.S4] — механизм изучен, можно использовать

Все 4 "перспективных" технических направления методички (Q∩T, cohomology, MITM, ANF ext) прощупаны и закрыты эмпирически. Wang extension — последний технический кандидат; если он тоже закрыт структурно, программа упирается в plateau.
...
> TL;DR: Открыто: |Sol_17| плотность, extension Wang>r=17, ANF early-verify extension. Закрыто: T_BOOMERANG_INFEASIBLE, T_ROTATIONAL_NEGATIVE, T_MILP_INFEASIBLE_17, Wang в мультиблоке, c-мир (P=2^-77), Hensel/Newton, побитовые маячки, GPK дифф. сигнал. **Scoping-закрыто 2026-04**: MITM O(2⁸⁰), Q∩T framework, cohomology probe — все empirically validated как не дающие преимущества (см. §II.8.3).
...
**Q_MITM_STATE16** ⊘SCOPED 2026-04 [§II.8.3, research/qt_minimal/]: MITM через state[16] = O(2^80) — не full-collision атака. По самой методичке (v20 Раздел 88 П-210..П-214): *"чистый МИТМ по δstate[17] стоит O(2^80) — хуже Wang chain"*; *"birthday collision по δstate ≠ SHA-256 коллизия"*; min HW(δstate[64])=91 (не 0). Реализация не стоит траты времени — не даёт ничего сверх Wang-chain.
...
**Q_QT_SOLVER** ⊘SCOPED 2026-04 [§II.8.3, research/qt_minimal/]: "Q∩T framework" как отдельная алгебра закрыт эмпирически. z3 BitVec SMT **уже является де-факто Q∩T-решателем** — внутренне обрабатывает ADD (T-часть, carry-cascade theory propagation) и AND/XOR (Q-часть, bit-blast CDCL). На mini-SHA (n=16, R=4..6): полный preimage за 5-30 мс, phase transition на R=8 (TIMEOUT 120с). Проблема с full SHA-256 — scale bit-blast'а, не отсутствие Q∩T-логики. Наивный "2^144" из §II.7.10 — это теоретический bound для послойного подхода, не для SMT, и не улучшается "новой алгеброй" на тех же примитивах.
...
## §II.8.3 Scoping-закрытые 2026-04 (⊘SCOPED)

Три направления, которые казались перспективными по методичке, после empirical scoping'а на mini-SHA (research/qt_minimal/) закрыты. Не повторять.

**⊘SCOPED Q_QT_FRAMEWORK**: Q∩T как новая алгебра. Эмпирически: z3 BitVec решает mini-SHA preimage (n=16, R=4..6, полный хеш) за 5-30 мс; phase transition на R=8 (full-hash TIMEOUT 120s). SMT уже реализует Q∩T через bit-vector theory. Проблема — scale, не алгоритм. 60-66% constraints линейны, но ручная разметка solver'у не даёт ускорения. [FINDINGS.md]

**⊘SCOPED Q_MITM_STATE16**: MITM O(2⁸⁰). Сам v20 (Раздел 88) говорит "хуже Wang chain"; даёт state[17] near-collision, не full SHA-256 collision; min HW(δstate[64])=91 даже с Wang+MITM. Template для понимания δstate структуры, не атака.

**⊘SCOPED Q_COHOMOLOGY_PROBE**: Карри-карта как 2-коцикл → искать H²-obstruction. Эмпирически измерен local Jacobian deficit over GF(2) на n=4..32, R=1..6 (20 anchors each). **Новая позитивная находка** ниже; для атаки — ничего сверх уже известного. [COHOMOLOGY_FINDINGS.md]

**⊘SCOPED Q_ANF_EARLY_VERIFY_EXTENSION**: Пробить 7.6× потолок §132 через ortho-stacking. Эмпирически: triple-stack checks на different bit positions (low + mid + high) дают **идеально независимые** pass rates (0.5^3 каждый → 0.5^9 joint). ✓ новый ortho-источник найден. НО стоимость carry-walk до позиции `p` = O(p) → spread стратегии всегда хуже contiguous low-k. Fundamental ceiling `speedup ≈ V/L` где V=стоимость full-verify, L=bit width. Для real SHA-256 при V≈100: speedup ≈ 13×. Методичка §133 сама уже корректно зафиксировала ~9× full recall потолок. Алгоритмического прорыва тут не будет — ограничение архитектурное (carry sequential). [ANF_FINDINGS.md, anf_optimality.py]

**∆EXP Q_WITT_PRISMATIC_LIFT** [2026-04, research/qt_minimal/CROSS_POLLINATION.md]: **Cross-pollination attempt** — формализация mini-SHA в языке Witt-vectors над F_2. **Новый теоретический результат**:

**T_FILTRATION_PURE_ROTATIONS** ⚡VER: В Witt-filtration F_k = 2^k·Z/2^n, **все** SHA-операции КРОМЕ rotations preserve F_k (100%). ROTR_r ломает F_k с rate 50-99% зависимо от r (coprime с n — 99%, commensurate — 50%).

**T_NOGO_LINEAR_WITT** ✓DOK: Не существует линейного F_2-базиса одновременно respecting Witt-filtration AND мapping rotations к filtration-preserving form. Причина: ROTR на F_2^n имеет минимальный полином (x+1)^n — один Jordan-блок, non-semisimple, единственное invariant subspace = 1-dim.

**Следствие**: security SHA-256 живёт ровно в **нelinear compatibility** между Z/2^n arithmetic (ADD) + F_2 boolean (Ch, Maj) + Galois structure (rotations). Нет basis где все три типа операций одновременно просты. Rotation — единственный ингредиент который превращает SHA из tractable в intractable.

**Target framework для будущих попыток**: prismatic cohomology (Bhatt–Scholze 2019) — единственная существующая математика, натурально видящая mixed-characteristic structure (F_p и Z_p одновременно через δ-ring formalism). **Никем пока не применялось к SHA**. Realistic research program = 5-10 лет с серьёзной командой.

Статус ∆EXP потому что это **identified target**, не эксплуатированный path. Session-level работа довела до точной формулировки obstruction, но не решила. [witt_vectors.py, witt_analysis.py, rotation_obstruction.py]

**Программа стартована session-by-session**: `research/prismatic/`. Session 1 (2026-04): δ-ring foundations, verified axioms on Z, Z/4. **Найдено первое препятствие**: Bool ring F_2[t]/(t²-t) имеет тривиальную δ-структуру (idempotence). Следствие: naive prismatic на SHA's natural ring = trivial. Нужна Witt-reformulation. Session 2 target: δ-compatibility SHA-operations на W_n(F_2). [research/prismatic/SESSION_1.md, README.md]

---

# 🔵 PRISMATIC PROGRAM — ИТОГ 69 СЕССИЙ (2026-04-25)

**Программа завершена в силу полноты ответа.** 69 sessions, 62 теоремы, 0 cryptanalytic advances. **Это — полный ответ**, не дефицит данных.

## Главный мета-вывод

**SHA-256 — точно скалиброванный артефакт инженерии**, а не "случайный шифр". Все известные классические attack vectors конвергируют на **один и тот же барьер ~46 раундов**, что **не случайность**, а следствие composition lemma + design calibration к 2^128.

### COMPOSITION LEMMA (мета-теорема, неформальная)

Per-round advantage любого классического метода атаки — **линейна или сублинейна** (per-round bias ≤ 1/4 для Walsh, AI = 1-2 per round, weak channels +1-3 raunda advantage). Per-round разрушение информации — **экспоненциально** (ANF degree 2^T, trail prob c^T при c < 1 после saturation T ≈ 11).
...
$$\text{AttackCost}(T = 64) \ge 2^{2 \cdot 64} = 2^{128} = \text{exact birthday для коллизий}$$

**SHA-256's 64 раунда настроены ровно на этот предел.** NIST не "поставил" числа — они вытекают из требования "защита на пределе brute-force против всех известных к 2001 году методов".

### CALIBRATION PRINCIPLE

Каждое число дизайна — параметр в этой калибровке:
- **64 раунда**: cumulative attack cost = brute force.
- **256 бит state**: birthday collision = 2^128.
- **8 регистров**: минимум для биективности (Session 51: 4-register вариант **не биективен** в 78% случаев).
- **ADD с carry**: единственная операция, ломающая 2-adic lifting (Theorem 40.1, Session 40).
- **Σ rotation constants (2,13,22) и (6,11,25)**: точная настройка nilpotency (Theorem 24.1: nilp = ⌈n/d⌉; Σ_1 = 11, Σ_0 = 32 — **разные** для разных регистров).

## 62 теоремы — accumulated structural infrastructure

Полный список с доказательствами/эмпирикой: `research/prismatic/SESSION_*.md`, `PRISMATIC_PROGRAM.md`. Ключевые классы:

### Class A: Linear algebra (Sessions 13-26, доказательные)
- **Theorem 24.1 (Lucas-XOR nilpotency)**: для M = ⊕_r (1+s)^r в F_2[s]/(s^n) с |R| odd, nilpotency = ⌈n/d⌉ где d = min{i > 0 : c_i = 1}, c_i = |{r : i&r = i}| mod 2. Объясняет deg(min poly Σ_1) = 11 vs Σ_0 = 32.
- **Theorem 25.1**: ord(R_lin) = 448 = 2^6 · 7 (для XOR-substituted раунда).
- **Theorem 29.1**: bare round R с K=W=0 имеет **уникальную** fixed point (zero state). Доказательство: M_fix = I + N_1·N_0, нилпотент в коммутативном F_2[s]/(s^32) ⇒ I + nilp имеет тривиальное ядро.
- **Theorem 30.1**: schedule operator S — cyclic dim 512, factorisation (z^6+z^5+1)·g_506.

### Class B: Polynomial / ANF (Sessions 27, 33, 39, доказательные)
- **Theorem 27.1**: quadratic structure dim 64, diagonal-index. Только 64 of 256 output bits нелинейны.
- **Theorem 33.1 (carry degree law)**: deg((x+y)_i) = i + 1, |ANF((x+y)_i)| = 2^i + 1. Доказательство по индукции на c_{i+1} = Maj(x_i, y_i, c_i).
- **Theorem 39.1**: 256 round-bit полиномов linearly independent в deg-≤2 кольце.

### Class C: Statistical / chaos (Sessions 34, 36, 38, 47-50, эмпирические)
- **Theorem 34.1 (Walsh)**: bias ≤ 1/4 per round (rank 4 quadratic forms на a'-битах).
- **Theorem 38.1**: avalanche per single bit flip = 5.06 ± 5.45 (vs ideal 128).
- **Theorem 47.1**: orbit gzip ratio = 0.328 (= register-shift redundancy 1/4 + overhead).
- **Theorem 48.1 (Lyapunov)**: λ ≈ 1.42 chaotic, T_sat ≈ 8 - log₂(d_0).
- **Theorem 49.1-2**: J_v всегда rank 256, quadratic noise floor 1.08% per state.
- **Theorem 50.1**: digest cloud Hamming distribution = Binomial(256, 0.5) (χ² passes).

### Class D: Negative results (Sessions 35, 41, 51, 52, 55, 62)
- **Theorem 35.1**: NO simple bit-permutation symmetries.
- **Theorem 41.1**: actual R (full ADD) has order ≫ 5000. Theorem 25.1 applies only to R_lin.
- **Theorem 51.1**: SHA bijectivity fragile — 4-register variant non-bijective.
- **Theorem 52.1**: J_v char poly profile matches random GL_256(F_2).
- **Theorem 55.1**: mini-SHA cloud имеет тривиальную persistent homology.
- **Theorem 62.1**: NO mod-q invariants (popcount mod 257 anomaly = artefact).

### Class E: Attack-oriented (Sessions 56-69, доказательные + эмпирические)
- **Theorem 53.1**: per-round AI = 1-2; composition raises to n/2 by T ≈ 7.
- **Theorem 59.1 (LLL infeasibility)**: monomial count 10^25 at T=4, beyond reach.
- **Theorem 66.1 (weak channels)**: register asymmetry — (c, d, h) slow propagation, (e, a) fast. Δ_in = low byte d → <d_1> = 5.38 (vs HW=8 random 27).
- **Theorem 67.1 (deterministic MSB transitions)**: Pr[R(x ⊕ e_{d,31}) ⊕ R(x) = e_{e',31}] = **1**. Pr[R(x ⊕ e_{h,31}) ⊕ R(x) = e_{a',31} ⊕ e_{e',31}] = **1**. Доказательство: ADD усекает carry уходящий из бита 31; флип MSB → ровно один выходной бит. **Единственная доказательная differential теорема в программе.**
- **Theorem 68.1 (greedy trail decay)**: cumulative trail probability — 8 раундов ~ 2^-50 (feasible attack), 12 раундов ~ 2^-100, 64 раунда ~ 2^-300.

## ❌ НЕ ПОВТОРЯТЬ — закрытые направления (Sessions 1-69)

Следующие подходы **исчерпаны** в нашей программе (с конкретными числами):

### Direct attacks
- **z3/SAT collision**: TIMEOUT при T ≥ 5-8 раундов (Session 56)
- **Boomerang**: underperforms direct differential на mini-SHA, in literature cap at 46 rounds (Session 57)
- **Rebound**: cost ~2^120 для full SHA, cap 46 rounds (Session 58)
- **LLL/XL/Gröbner**: monomial explosion 10^25 at T=4 (Session 59)
- **Multi-block random**: 10^5 trials → 0 collisions (Session 60)

### Structural attacks
- **Bit-permutation symmetries**: NONE (Sessions 35, 44 — после исправления бага)
- **Mod-q invariants**: NONE (Session 62)
- **Weak states / λ heterogeneity**: CV = 0.06, mostly uniform (Session 61)
- **TDA / persistent homology**: тривиальная топология (Session 55)
- **Joint compression (m, H(m))**: pairs compress as independent (Session 65)
- **Cycle structure**: order ≫ 5000, нет коротких циклов (Session 41)

### Indirect angles
- **Subspace mixing**: half-state slower (0.386), но это структурный bound, не exploit (Session 64)
- **Conservation laws**: единственная микро-аномалия register_sum mod 257 (3.5σ, прирост 3× к birthday — far from attack threshold)
- **Free-start collision**: T ≤ 4-5 trivially solvable, T = 5 timeout. Published academic 52 rounds (Mendel 2013)
- **2-adic lifting (prismatic)**: ROTR не truncation-compatible. Closed at Session 40

### Linear approximations (бывшие "успехи" что оказались артефактом)
- **R_lin (XOR substitute)**: ord = 448, nice algebra. **НЕ работает для real R** (Session 41/46 fix)
- **s-basis matrix application к x-basis битам**: BUG в Sessions 38-45, fixed in 46

## ✅ Что РАБОТАЕТ (можно использовать дальше)

1. **Carry chain analysis (Theorem 33.1)** — основа любого dieferential reasoning на ARX.
2. **Lucas-XOR nilpotency (Theorem 24.1)** — для дизайна / анализа других ARX hashes.
3. **Deterministic MSB transitions (Theorem 67.1)** — концrete handle для reduced-round attacks.
4. **Weak channel asymmetry (Theorem 66.1)** — (c, d, h) slow vs (e, a) fast.
5. **Saturation T ≈ 11** — universal constant (10+ метрик согласуются).

## Реалистичные пути после программы

1. **Применить infrastructure к ДРУГИМ hashes**: Lucas-XOR / carry analysis работают на любых ARX — SHA-1, SHA-3 (different структура), BLAKE2/3, KangarooTwelve.

2. **Quantum approaches** (open problem):
   - Grover gives sqrt speedup → 2^85 — недостаточно для 256-bit preimage
   - Нужен exp-speedup для коллизий — считается невозможным в bosonic models
   
3. **Поиск операций нарушающих Composition Lemma**: per-round advantage с **суперлинейной** компонуемостью. Никто не нашёл за 24 года. Может быть **математически невозможно** в текущих framework'ах — это новое направление теоретической криптографии.

4. **Side channels** — out of scope для математического анализа.

5. **Полноценная differential cryptanalysis на 24-46 reduced rounds** — требует Wang/Mendel-уровень expertise, годы работы. Известно что возможно, известно как, но цена — годы специалистской работы.

## Ссылки на накопленный материал

```
research/prismatic/
├── PRISMATIC_PROGRAM.md          # paper-style consolidation Sessions 1-22
├── README.md                      # таблица всех 69 sessions
├── SESSION_1.md ... SESSION_31.md # individual session notes (linear algebra)
├── SESSION_32.md ... SESSION_42.md # number theory + chaos + statistical
├── SESSION_43.md ... SESSION_55.md # wild directions + bug fix + TDA
├── SESSION_56_60_attacks.md       # 5 attack frameworks (all fail)
├── SESSION_61_63_inverse.md       # inverse cryptanalysis (mostly negative)
├── SESSION_64_66_indirect.md      # structured Δ — REAL findings (c/d/h slow)
├── SESSION_67_69_deepening.md     # DETERMINISTIC MSB transitions ★
└── session_*.py                   # 69 computational artefacts (verifiable)
```

## Принцип для будущих сессий

> **«Когда нет ответов — это самый главный ответ»** (user, 2026-04-25)
> 
> SHA-256 unbreakable классическими методами **не из-за нехватки попыток**, а **из-за математически точной калибровки**. 69 сессий доказали это через **convergence of negative results** — все методы разбиваются о один и тот же барьер.
> 
> **Не атаковать SHA-256 дальше теми же методами**. Использовать infrastructure для других задач или искать **principially new mathematics** (operations violating Composition Lemma) — что является открытой теоретической проблемой.

---



**⊘SCOPED Q_CARRY_LINEAR_INVARIANT**: Истинное exploration — ищем structural handle в carry kernel. 4 последовательных probe:
- **Persistent kernel**: intersection ker(J_r) across rounds = trivially MSB + state-dep extras. Не глобальный invariant.
- **Kernel distribution**: per-round kernel dim ≈ `n·(R_total - r - 1) + intrinsic_const` — тривиальная causality (future-W бит не affect past-r carries).
- **Total invariants**: 0 total invariants (flip без изменения output). Хорошая news для SHA.
- **Linear invariant**: **100% state-independent** — любой flip `v ∈ ker(J_carry[A]) ∩ ker(J_carry[B])` даёт **детерминированный output diff = {register 0 bit k, register 4 bit k}** где k = позиция flip'а в W_last.

**Новое формулирование известного**: `a' = T1 + T2`, `e' = d + T1`. Flip in W_last с сохранением carry → T1 меняется 1 bit → a' и e' меняются тот же bit. Это **единая основа** T_DCH_EXACT (§II.2.4) + Wang-chain (§II.4) + T_STATE17_STRUCTURE (§II.4.11) — все три явления суть проявления этой линейной симметрии.

Не новый attack vector (Wang-chain уже эксплуатирует). Но более прозрачная формулировка, объясняет WHY r=17 barrier: carry kernel исчерпан в точности на том раунде, где schedule перестаёт давать "свежие" W. [EXPLORATION_FINDINGS.md, linear_invariant.py]

### T_JACOBIAN_DEFICIT_R_INVARIANT ⚡VER [2026-04, research/qt_minimal/deficit_scaling.py]
Локальный rank-deficit carry-Jacobian **R-независим** (закон сохранения через раунды):
| n | mean deficit (R=1,2,4,6) |
|---|---|
| 4 | 1.38 |
| 8 | 1.95 |
| 16 | 2.73 |
...
Для n=32 (real SHA-256): deficit ≈ 4.84, после вычета тривиальной MSB-компоненты (≈1) ≈ 3.8 — **консистентно с §II.7.7** (carry-rank 589/592 = deficit 3).

Ограничения: ~90% anchor'ов имеют kernel dim=1, и это MSB последнего W слова (арифметика mod 2^n). Дополнительные kernel-направления state-dependent (Wang territory). **Не даёт нового вектора атаки**, но формализует "почему Wang работает детерминированно": carry obstruction bounded, не растёт с R.

## §II.8.4 Отозванные артефакты (⊘ROLL) — расширено
...
## Линия D: Scoping session (2026-04, research/qt_minimal/)

Сессия по валидации трёх "перспективных" направлений из `06_open_questions.md`. Все три ⊘SCOPED.

### D-1: Q∩T framework probe
- mini-SHA параметризованная (n-бит слов, R раундов, Σ/σ/Ch/Maj/ADD как в SHA-256)
- 4 solver'а: brute, SAT bit-level, SAT BitVec, naive Q+T split
- Находка: 60-66% constraints линейны over GF(2) независимо от (n, R)
- z3 BitVec решает mini-SHA preimage (n=16, R=4..6) за 5-30 мс
- Phase transition: R=8 full-hash = TIMEOUT 120с
- **Вывод**: z3 bit-vector theory IS Q∩T solver. Ручная "своя алгебра" не даст преимущества.

### D-2: Cohomology probe (carry Jacobian)
- Local Jacobian carry-map над GF(2), 20 anchor'ов, n=4..32, R=1..6
- Deficit R-INVARIANT: для n=16 deficit ≈ 2.7 при R=1,2,4,6; для n=32 ≈ 4.84
- ~90% anchor'ов: kernel dim=1 = MSB последнего W слова (trivial mod 2^n)
- Остальные kernel-направления state-dependent = Wang territory
- **Вывод**: подтверждает §II.7.7 (589/592), но не даёт новой атаки.

### D-3: MITM O(2⁸⁰) scoping
- Прочитан оригинальный v20 Раздел 88 (П-210..П-214)
- Сама методичка: "чистый МИТМ по δstate[17] стоит O(2^80) — **хуже Wang chain**"
- min HW(δstate[64])=91 — не full collision
- **Вывод**: реализация не нужна, template для понимания δstate структуры, не атака.

### D-4: ANF early-verify extension (§132 beyond 7.6×)
- Реализован toy model L=16 simplified round `y = M·x + K + C(x,K)`
- Baseline §132 воспроизведён: ~3× при V=L cost model, ~13× при V=100 (соответствует §133 ~9×)
- **Новая находка**: triple-stacking ortho-checks на разных bit positions (low+mid+high) дают **идеально независимые** pass rates (exact 0.5^3 per check, joint 0.5^9)
- НО стоимость carry-walk до позиции p = O(p) → spread всегда хуже contiguous low-k
...
- Реализовали Witt-vector arithmetic W_n(F_2), подтвердили W_n(F_2) ≅ Z/2^n as rings
- Filtration analysis: **все** boolean/arithmetic операции (ADD, XOR, AND, Ch, Maj) preserve F_k; **только** rotations ломают filtration (50-99% rate)
- **No-go для linear basis**: ROTR non-semisimple над F_2 (один Jordan-блок), нет fix через линейное изменение базиса
- **Identified target framework**: prismatic cohomology (Bhatt–Scholze) — единственная существующая mathematics видящая mixed-characteristic structure, которая нужна для SHA. Никем не применялась к крипто.
...
- `research/qt_minimal/equations.py` — Q/T генератор constraints
- `research/qt_minimal/solvers.py` — 4 solver'а
- `research/qt_minimal/cohomology_probe.py`, `kernel_analysis.py`, `deficit_scaling.py`
- `research/qt_minimal/anf_early_verify.py`, `anf_optimality.py` — toy §132 + cost analysis
- `FINDINGS.md`, `COHOMOLOGY_FINDINGS.md`, `ANF_FINDINGS.md` — полные отчёты

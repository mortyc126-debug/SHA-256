/*
 * SP at BELOW-threshold ratio: test if SP works when instances are SAT
 * =====================================================================
 *
 * At α=4.267 (threshold): ~50% UNSAT → ceiling on solve rate
 * At α=4.0: almost all SAT → if SP works, should get ~100%
 * At α=3.5: very easy, WalkSAT alone solves them
 *
 * This tells us: is SP's failure at n=1000+ due to UNSAT instances
 * or actual SP failure?
 *
 * Compile: gcc -O3 -march=native -o sp_below sp_below_threshold.c -lm
 */
...
int main(void){
    printf("═══════════════════════════════════════════════════\n");
    printf("SP BELOW THRESHOLD: α=4.0 vs α=4.267\n");
    printf("═══════════════════════════════════════════════════\n\n");

    double ratios[] = {3.5, 4.0, 4.1, 4.2, 4.267};
    ...
    for(int ri=0; ri<5; ri++) {
        ...
        for(int ti=0; ti<4; ti++) {
            int nn=test_n[ti];
            int ni=(nn<=200)?10:(nn<=500?5:(nn<=1000?5:3));
            ...
            printf("  %6d | %2d/%2d | %4d   | %3.0f%% (%.0fms)\n",
                   nn,solved,total,solved,100.0*solved/total,tms/total);
            fflush(stdout);

            if(tms/total > 60000 && solved == 0) {
                printf("    (too slow, stopping)\n");
                break;
            }
        }
        printf("\n");
